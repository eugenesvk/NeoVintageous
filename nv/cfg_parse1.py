...
def _parse_general_g_kdl1(general_g:kdl.Node):
    win = sublime.active_window()
    ...
    for node in general_g.nodes: # set relativenumber=true
        _parse_general_cfg_kdl1(general_cfg=node,st_pref=st_pref)
def _parse_general_cfg_kdl1(general_cfg:kdl.Node,st_pref=None) -> None:
...
def _parse_vars_kdl1(node_vars:kdl.Node,var_d:dict={}):
...
def _parse_keybinds_kdl1(keybinds:kdl.Node,var_d:dict={}):
    var_d_combo = _parse_vars_kdl1(keybinds,var_d)
    for kb_node in keybinds.nodes: # (Ⓝ)"q" "OpenNameSpace"
        _parse_keybind_kdl1(keybind=kb_node, var_d=var_d_combo)
def _parse_keybind_kdl1(keybind:kdl.Node, gmodes:Mode=Mode(0), var_d:dict={}):
    ...
    if children and not isChain:       # without Chain argument...
        for child in children:         # ...parse children as keybinds
            _parse_keybind_kdl1(keybind=child, gmodes=modes, var_d=var_d)
