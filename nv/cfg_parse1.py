...
def _parse_keybind_kdl(keybind:kdl.Node, CFG:dict, cfgU, gmodes:Mode=Mode(0),var_d:dict={}):
    ...
    for dkey,key_abbrev in _keybind_prop.items():
      ...
      if pkey not in key_abbrev: # non-specified key=val pairs
        if isinstance(val,float) and val.is_integer():
          ...
          prop_rest[pkey] = val
  (cmd,cmdo,isChain)    = _parse_keybind_arg1(node=node, CFG=CFG, prop_subl=prop_rest) # Parse arguments
  cmd_txt.extend(cmd )
  cmd_o  .extend(cmdo)
    ...
    for child in children:         # ...parse children as commands
      ...
      for pkey,tag_val in child.props.items(): #
        ...
        for dkey,key_abbrev in _keybind_prop.items():
          if pkey not in key_abbrev: # non-specified key=val pairs
            prop_rest[pkey] = val
      (cmd,cmdo,_) = _parse_keybind_arg1(node=child, CFG=CFG, prop_subl=prop_rest)
      cmd_txt.extend(cmd )
      cmd_o  .extend(cmdo)
