...
def parse_kdl_config(v,cfg:str, cfg_p:Path, kdl_docs:list    , enclose_in:str=''    ,var_d:dict={}):
  ...
  for node in list(nodes_iter): # Check imports (in children if enclosed)
    if node.name in ["import"] and node.type_annotation is None: # match untagged import node ()
      nodes_iter.remove(node) # consume imports, successfull will be stored as separate docs
      # import_var = {}
      ...
      for arg in node.args: # import (keybind)"./NV/mykeys.kdl"
        ...
        if (cfg_import_f := Path(cfg_p.parent,fname).expanduser()).exists():
          ...
          break
        parse_kdl_config(v,cfg_import, cfg_import_f, kdl_docs, enclose_in=tag, var_d=var_d_new)
  kdl_docs += [(doc,var_d)]
  return (doc,var_d)
