    ...
    def run(self, text_cmd=None, count=None, do_eval=True):
        ...
        try:
            if _LT:
                ...
                count_ = count or '_'
                _log.keyt('\n\n—⌨️T——\nHFeed ‘%s’      #%s eval=%s             reg‘%s’ C‘%s’ seq‘%s’ P‘%s’ txtP‘%s’ mot‘%s’⋅%s act‘%s’⋅%s'
                ,                             text_cmd,count_,do_eval,         reg, regC,    seq, seqP,    txtP,    mot,  mot_c,act,act_c)
...
class nv_vi_search_forward(TextCommand):
    def run(self, edit, pattern='', **kwargs):
