            ...
            if (node := cfg.get(cfg_key,None)): # line "━" node/arg pair
                ...
                for i,(arg,tag,val) in enumerate(cfgU.cfg_parse.arg_tag_val(node)):
                    ...
                    if i == 0:
                        ...
                        CFG[node.name] = val #;_log.debug('indicator ls from arg @%s %s',node.name,val)
                    elif i > 0:
                ...
                if not args:
                    _log.warn("node ‘%s’ is missing arguments in its child ‘%s’"
                        ,         cfg_key,                               node.name)
        node = cfg
        for i,(key,tag_val,tag,val) in enumerate(prop_key_tag_val(node)): # line="━", alt notation to child node/arg pairs
            if tag:
                _log.warn("node ‘%s’ has unrecognized tag in property ‘%s=%s’",node.name,key,tag_val)
            if key in CFG:
                CFG[key] = val ;_log.debug("indicator ls from property ‘%s=%s’",key,val)
            else:
