            ...
            if (node := cfg.get(cfg_key,None)): # prefix "⌗" node/arg pair
                ...
                if not args:
                    _log.warn("node ‘%s’ is missing arguments in its child ‘%s’"
                        ,         cfg_key,                               node.name)
        node = cfg
        for i,(key,tag_val,tag,val) in enumerate(prop_key_tag_val(node)): # prefix="⌗", alt notation to child node/arg pairs
            if tag:
...
def must_collect_input(view, motion: ViMotionDef, action: ViOperatorDef) -> bool:
    _log.debug("must_collect_input? mtn‘%s’⎀?%s‘%s’ act‘%s’⎀?%s‘%s’"
